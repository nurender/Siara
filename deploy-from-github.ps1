...
Write-Host "🚀 Deploying Siara Events from GitHub..." -ForegroundColor Cyan
Write-Host "📦 Repository: $REPO_URL" -ForegroundColor Yellow
Write-Host "🖥️  Server: ${SERVER_USER}@${SERVER_HOST}" -ForegroundColor Yellow
Write-Host ""
...
Write-Host "   Frontend: http://170.64.205.179:3000" -ForegroundColor Yellow
Write-Host "   Backend API: http://170.64.205.179:5000/api/health" -ForegroundColor Yellow
Write-Host ""
Write-Host "📊 To check status, run:" -ForegroundColor Cyan
Write-Host "   ssh ${SERVER_USER}@${SERVER_HOST} 'pm2 status'" -ForegroundColor Yellow
