...
import { getSettings, getPage, getBlogs } from '@/lib/cms';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: page?.meta_title || `Blog | ${siteName} - Wedding Planning Insights`,
    description: page?.meta_description || 'Expert wedding planning tips, venue guides, and inspiration from Rajasthan\'s premier wedding planners.',
    keywords: 'wedding blog, wedding planning tips, Rajasthan wedding venues, wedding inspiration, wedding trends',
    openGraph: {
      title: page?.meta_title || `Blog | ${siteName}`,
      description: page?.meta_description || 'Expert wedding planning tips, venue guides, and inspiration.',
      type: 'website',
...
async function getPageData(page: number = 1) {
  try {
    const [pageData, blogsResponse, settings] = await Promise.all([
      getPage('blog'),
      getBlogs({ limit: 6, page }),
      getSettings(),
    ]);
...
export default async function BlogPage({ searchParams }: BlogPageProps) {
  ...
  const { sections, relatedData } = await getPageData(currentPage);
        ...
        <section className="relative min-h-[80vh] flex items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl mx-auto px-6 lg:px-8 text-center py-32">
            ...
            <p className="font-dm-sans text-lg md:text-xl text-siara-purple-200/80 max-w-2xl mx-auto mb-10">
              Our blog content is being prepared. Please visit the admin panel to add sections.
            </p>
